...
async function main() {
  ...
  try {
    ...
    const submitted = await evaluate<{ ok: boolean; step: number; detail: string }>(cdp, `(async () => {
      ...
      const set = (name, value) => { const el = document.querySelector('[name="' + name + '"]'); if (!el) return false; const proto = el.tagName === 'SELECT' ? HTMLSelectElement.prototype : el.tagName === 'TEXTAREA' ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype; Object.getOwnPropertyDescriptor(proto, 'value').set.call(el, value); el.dispatchEvent(new Event('input', { bubbles: true })); el.dispatchEvent(new Event('change', { bubbles: true })); return true; };
      ...
      const firstOption = (name) => { const s = document.querySelector('select[name="' + name + '"]'); return s ? [...s.options].find((o) => o.value)?.value : ''; };
      ...
      await raf(); if (!click('Continue') && !click('Next')) return { ok: false, step: 1, detail: 'no continue button' }; await raf(); await raf();
      ...
      await raf(); if (!click('Continue') && !click('Next')) return { ok: false, step: 2, detail: 'no continue button' }; await raf(); await raf();
    ...
    record("public:apply", "three-step application submits and persists", submitted.ok && !!row ? "PASS" : "FAIL", `${submitted.detail.replace(/\s+/g, " ").slice(0, 120)} · db=${!!row}`, submitted.ok && row ? undefined : "PUBLIC-APPLY");
