...
const AUDIT = `(() => {
  ...
  const clipped = [...document.querySelectorAll('h1,h2,h3,.v9-tag,.v9-eyebrow,strong,button,a')].filter(e => !e.classList.contains('v9-visually-hidden') && e.scrollWidth > e.clientWidth + 2 && ['hidden','clip'].includes(getComputedStyle(e).overflowX) && e.getBoundingClientRect().width > 0 && !inScroller(e)).slice(0, 5).map(e => (e.textContent || '').trim().slice(0, 30));
