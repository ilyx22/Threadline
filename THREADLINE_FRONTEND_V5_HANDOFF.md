...
**Phone pass (25 September 2026, DEC-036).** After the freeze, the owner reopened the phone experience ("too narrow, some stuff hard to read"). The audit and every fix are in `docs/site/PHONE_AUDIT_2026-09-25.md`; the rules sit at the end of `src/styles/marketing-v9/index.css` under `max-width: 759px` (plus one rule under 599px for the bench labels). Desktop and tablet are untouched. The public wordmark is now the product identity from `src/components/brand/logo.tsx` (thread mark plus tracked uppercase name), 20% larger and heavier than the product default; the old serif wordmark with the drawn thread is gone. A second pass on 26 September (the owner's notes; second table in the audit document) removed the duplicate "What you do" tile everywhere, replaced the two fit lists with one table (`src/components/marketing-v9/FitTable.tsx`, also on Who it is for), added a once-per-session intro (`src/components/marketing-v9/Intro.tsx`, mounted in the marketing layout with an inline pre-paint script that reads `sessionStorage['tl-intro']`), and on phones re-cropped the hero, closed the gap under the two rooms, made the route one wrapping line, swapped the learning paragraph for five lines (`learning.steps`), enlarged the bench drawing in a sliding stage, and closed the space above the closing scene.
