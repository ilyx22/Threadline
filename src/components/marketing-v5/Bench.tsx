...
import { Artefact, At, C, Grain, Person, SKIN, outline as O } from "./art/kit";
    ...
    <div className="v5-bench" data-state={state} data-applied={applied ? "true" : "false"}>
      ...
      <div className="v5-bench-stage">
        <svg viewBox="0 0 900 520" className="v5-art" role="img" aria-label={`The testing bench. The piece sits on five blocks: idea, hook, distribution, audience, destination. Three measuring jars, ${c.gauges.join(", ")}, each with an expected mark. ${state >= 1 ? "The actual level is shown." : ""} ${state >= 2 ? `The ${c.failing} block has tipped.` : ""} ${state >= 3 && applied ? `The ${c.failing} block has been replaced.` : ""} ${retest ? "The jars have been read again." : ""}`}>
          <Grain id="g-bench" />
          <rect x={0} y={0} width={900} height={520} rx={30} fill={C.mint} />
          {/* the bench */}
          <rect x={40} y={400} width={820} height={22} rx={8} fill={C.wood} {...O} />
          <path d="M80 422 V500 M820 422 V500" stroke={C.ink} strokeWidth={9} strokeLinecap="round" />
          {/* the hook above the bench */}
          <path d="M100 0 V60 H330" fill="none" stroke={C.ink} strokeWidth={7} strokeLinecap="round" />
          <g className="v5-hook">
            <path d="M330 60 V150" stroke={C.ink} strokeWidth={5} strokeLinecap="round" />
            <path d="M330 150 q0 22 -16 22 q-12 0 -12 -12" fill="none" stroke={C.ink} strokeWidth={5} strokeLinecap="round" />
          </g>
          ...
          <At x={330} y={400}>
              ...
              return (
                <g key={name} className={`v5-block${failed && !replaced ? " is-failed" : ""}${replaced ? " is-new" : ""}`} style={{ ["--bx" as string]: `${COMPONENT_X[i]}px` }}>
                  <g transform={`translate(${COMPONENT_X[i]} 0)`}>
                    <rect x={-35} y={-44} width={70} height={44} rx={6} fill={replaced ? C.mintDeep : failed ? C.coral : C.paper} {...O} />
                    <text x={0} y={-17} textAnchor="middle" className="v5-label" style={{ fontSize: 8.5, letterSpacing: "0.04em" }}>
                      {name.toUpperCase()}
                    </text>
                  </g>
                </g>
            ...
            <g className="v5-piece">
              <rect x={-190} y={-58} width={380} height={14} rx={5} fill={C.lilac} {...O} />
          ...
          {c.gauges.map((g, i) => {
            ...
            return (
              <g key={g} transform={`translate(${x} 400)`}>
                <rect x={-30} y={-230} width={60} height={230} rx={12} fill={C.white} {...O} opacity={0.95} />
                <rect x={-24} y={-6 - val * 2.1} width={48} height={val * 2.1} rx={6} fill={short ? C.coral : over ? C.butter : C.mintDeep} className="v5-fill" />
                <g transform={`translate(30 ${-6 - exp * 2.1})`}>
                  <path d="M0 0 H24 L30 -8 L24 -16 H0 Z" fill={C.gold} {...O} strokeWidth={2.4} />
                  <path d="M-30 0 H0" stroke={C.ink} strokeWidth={2.4} strokeDasharray="4 3" />
                </g>
          ...
          {/* the inspector */}
          <Person x={838} y={498} s={0.95} shirt={C.coral} apron={C.gold} skin={SKIN[4]} hairStyle="bun" glasses armL={[-60, -112]} armR={[30, -100]} look={-1} mood={showWhy && !retest ? "oh" : "smile"} />
          <circle cx={776} cy={384} r={16} fill={C.sky} {...O} />
          <path d="M788 396 l14 14" stroke={C.ink} strokeWidth={5} strokeLinecap="round" />
          <rect width={900} height={520} rx={30} filter="url(#g-bench)" opacity={0.3} pointerEvents="none" />
        </svg>
      </div>
