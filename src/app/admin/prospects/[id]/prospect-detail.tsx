...
import { recordScriptUseAction } from "@/lib/actions/sales-scripts";
...
export type ScriptOption = { id: string; key: string; version: number; context: string; stage: string };
  ...
  return (
    <Card>
      ...
      <CardBody className="space-y-5">
        ...
        {calls.map((call) => (
          <CallCard key={call.id} call={call} scripts={scripts} />
        ))}
      </CardBody>
    </Card>
  );
...
function CallCard({ call, scripts }: { call: CallView; scripts: ScriptOption[] }) {
  ...
  return (
    <div className="rounded-lg border border-line">
      ...
      <div className="space-y-4 px-4 py-4">
        ...
        {!done ? <ScriptUse callId={call.id} scripts={scripts} /> : null}
...
/**
 * Canonical script blocks on a live call. The operator picks a block; the
 * exact text and version are frozen against the call. With nothing approved
 * yet, the panel says so rather than offering wording nobody signed off.
 */
function ScriptUse({ callId, scripts }: { callId: string; scripts: ScriptOption[] }) {
  const [scriptId, setScriptId] = React.useState(scripts[0]?.id ?? "");
  const [pending, start] = React.useTransition();
  if (scripts.length === 0) {
    return (
      <p className="rounded-md border border-warning/30 bg-warning-soft px-3 py-2 text-[12px] text-warning">
        CANONICAL COPY IMPORT REQUIRED — no approved script block exists. Approve blocks under Admin → Scripts before using wording on a call.
      </p>
    );
  }
  return (
    <div className="flex flex-wrap items-end gap-2">
      <Field label="Use a canonical script block" htmlFor={`script-${callId}`} className="min-w-[260px] flex-1">
        <NativeSelect id={`script-${callId}`} value={scriptId} onChange={(e) => setScriptId(e.target.value)}>
          {scripts.map((s) => (
            <option key={s.id} value={s.id}>
              {s.stage.replace(/_/g, " ")} · {s.key} v{s.version}
            </option>
          ))}
        </NativeSelect>
      </Field>
      <Button
        ...
        onClick={() =>
          start(async () => {
            const r = await recordScriptUseAction(callId, scriptId);
